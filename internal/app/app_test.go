...
func TestApp(t *testing.T) {
	testCases := []struct {
		...
		{
			"permissive defaults no enablements",
			&app.Config{
				User:    "chronos",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			a := app.NewWithID(tc.id, new(stub))
...
func (s *stub) LookupEnv(key string) (string, bool) {
	switch key {
	case "SHELL":
...
func (s *stub) TempDir() string {
	return "/tmp"
}

func (s *stub) LookPath(file string) (string, error) {
...
func (s *stub) Executable() (string, error) {
	return "/home/ophestra/.nix-profile/bin/fortify", nil
}

func (s *stub) Lookup(username string) (*user.User, error) {
...
func (s *stub) ReadDir(name string) ([]fs.DirEntry, error) {
	switch name {
	case "/":
...
func (s *stub) Stat(name string) (fs.FileInfo, error) {
	switch name {
	case "/var/run/nscd":
		return nil, nil
	default:
...
func (s *stub) Open(name string) (fs.File, error) {
	switch name {
	default:
...
func (s *stub) Exit(code int) {
	panic("called exit on stub with code " + strconv.Itoa(code))
}

func (s *stub) Paths() internal.Paths {
